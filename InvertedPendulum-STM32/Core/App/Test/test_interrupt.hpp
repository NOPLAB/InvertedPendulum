...
class MockAdcHandler : public IAdcInterruptHandler {
 public:
  ...
  MockAdcHandler(MockADC_HandleTypeDef* handle)
      : adc_handle(handle), interrupt_called(false) {}

  ADC_HandleTypeDef* adcHandlerType() const override {
    return (ADC_HandleTypeDef*)adc_handle;
  }

  void handleAdcInterrupt() override { interrupt_called = true; }
};
...
class MockTimerHandler : public ITimerInterruptHandler {
 public:
  ...
  MockTimerHandler(MockTIM_HandleTypeDef* handle)
      : timer_handle(handle), interrupt_called(false) {}

  TIM_HandleTypeDef* timerHandlerType() const override {
    return (TIM_HandleTypeDef*)timer_handle;
  }

  void handleTimerInterrupt() override { interrupt_called = true; }
};
