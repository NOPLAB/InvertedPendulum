...
class TestableMotors {
 ...
 private:
  ...
  void setTimerCompare(MockTIM_HandleTypeDef* htim, uint32_t channel,
                       uint32_t compare) {
...
TEST_FUNCTION(motor_forward_direction) {
  ...
  ASSERT_EQ(1599,
            motors.getTimerCompare(&mock_htim2, 2));  // 0.5 * 3199 = 1599.5
  ASSERT_EQ(0, motors.getTimerCompare(&mock_htim3, 4));
...
TEST_FUNCTION(motor_pwm_calculation) {
  ...
  ASSERT_EQ(799,
            motors.getTimerCompare(&mock_htim2, 2));  // 0.25 * 3199 = 799.75

  motors.setSpeedLeft(0.75f);
  ASSERT_EQ(2399,
            motors.getTimerCompare(&mock_htim2, 2));  // 0.75 * 3199 = 2399.25
