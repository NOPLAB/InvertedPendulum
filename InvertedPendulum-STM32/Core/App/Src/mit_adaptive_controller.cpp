...
MitAdaptiveController::MitAdaptiveController(float dt, float gamma1, float gamma2, 
                                             float gamma3, float gamma4) 
    : dt(dt), prev_reference_output(0.0f), prev_reference_velocity(0.0f),
      prev_plant_output(0.0f), prev_plant_velocity(0.0f),
      adaptation_error(0.0f), reference_output(0.0f), plant_output(0.0f),
      error_integral(0.0f), prev_error(0.0f) {
...
void MitAdaptiveController::setInitialGains(float k1, float k2, float k3, float k4) {
  initial_gains[0] = k1;
  initial_gains[1] = k2;
  initial_gains[2] = k3;
  initial_gains[3] = k4;
...
float MitAdaptiveController::update(float x, float dx, float theta, float dtheta, float reference) {
  ...
  float control_input = -(adaptive_gains[0] * x + 
                         adaptive_gains[1] * dx + 
                         adaptive_gains[2] * theta + 
                         adaptive_gains[3] * dtheta);
...
void MitAdaptiveController::updateAdaptiveGains(float x, float dx, float theta, float dtheta) {
  ...
  for (int i = 0; i < 4; i++) {
    float adaptation_rate = gamma[i] * adaptation_error * sensitivity_derivatives[i];
...
float MitAdaptiveController::calculatePlantOutput(float x, float dx, float theta, float dtheta) {
...
void MitAdaptiveController::setAdaptationRates(float gamma1, float gamma2, float gamma3, float gamma4) {
  gamma[0] = gamma1;
  gamma[1] = gamma2;
  gamma[2] = gamma3;
  gamma[3] = gamma4;
